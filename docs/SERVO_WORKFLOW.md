...
**Tracking `main` is a deliberate trade.** It buys a set of panic and correctness
fixes (`RefCell` borrow hazards including one in `ResizeObserver`, a Taffy
conversion panic, flex/float layout bugs, HTTP `stale-while-revalidate`, Harfbuzz
shaping) at the cost of an unfiltered tree: no curation, no backports, and
...
`decodeAudioData` are still unimplemented. Reassess when `release/v0.5` is cut.
...
- **Dependency pins can collide.** `main` has moved to the released RustCrypto
  stack, while the `0.4` release tree needs
  `primeorder = "=0.14.0-rc.14"` (servo pins `p256/p384/p521 =0.14.0-rc.14`, and
  the released `primeorder 0.14.0` adds a `WnafSize` bound the rc curves don't
  implement). The pin has to be present for `retsurf-0.4` and absent for
  `retsurf-main`, or resolution fails outright.
- A `[patch]` only applies if the source's version still satisfies our
  requirement. `main`'s workspace version is `0.4.0` today; when upstream bumps
...
- Expect API drift to break our code, though it need not: measured 2026-07-28,
  ...
  **zero** changes to retsurf, in 10m34s, and both engine fixes still held at
  runtime. Do each move on a branch anyway, and keep the fallback line current.
