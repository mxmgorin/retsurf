...
//! audio thread only drains the PCM queue.
...
impl SdlAudioPlayer {
    pub fn new(id: usize, stream_type: StreamType, observer: GenericCallback<PlayerEvent>) -> Self {
        Self::with_sink(
            id,
            stream_type,
    ...
    fn with_sink(id: usize, stream_type: StreamType, events: Box<dyn EventSink>) -> Self {
        let shared = Arc::new(Shared::new(events));
        spawn_decoder(shared.clone(), stream_type);
        Self {
            id,
            stream_type,
            shared,
...
impl Player for SdlAudioPlayer {
    ...
    fn set_video_track(&self, _: i32, _: bool) -> Result<(), PlayerError> {
        Err(PlayerError::SetTrackFailed)
    }
}
...
/// `canPlayType` and `<source type>` filtering. Probably for what symphonia
/// demuxes and decodes outright, Maybe for containers that may hide an
/// unsupported codec, No for everything else — video included, there is no
/// video path at all.
pub fn can_play_type(media_type: &str) -> SupportsMediaType {
    ...
    if codecs.is_empty() {
        return match container {
            ...
            // The container may hold a codec symphonia has no decoder for
            // (opus in ogg, alac in mp4).
            Container::Ogg | Container::Mp4 => SupportsMediaType::Maybe,
        };
    }
...
fn spawn_decoder(shared: Arc<Shared>, stream_type: StreamType) {
    let result = thread::Builder::new()
        .name("audio-player".into())
        .spawn(move || decoder_thread(shared, stream_type));
    if let Err(e) = result {
...
fn decoder_thread(shared: Arc<Shared>, stream_type: StreamType) {
    ...
    match run_pipeline(&shared, source, seekable) {
        Ok(()) => {}
        Err(message) => {
...
fn run_pipeline(shared: &Arc<Shared>, source: ByteReader, seekable: bool) -> Result<(), String> {
    ...
    // Preroll one decoded packet: it pins the true sample rate before anything
    // is announced, and proves the file actually decodes.
    ...
    let (rate, channels, first_secs) = loop {
        ...
        match decoder.decode(&packet) {
            Ok(audio) if audio.frames() > 0 => {
                let spec = audio.spec();
                let rate = spec.rate();
                let channels = audio.num_planes();
                audio.copy_to_vec_interleaved::<f32>(&mut interleaved);
                let secs = packet_secs(&packet.pts, time_base, rate);
    ...
    let duration_secs = num_frames
        .map(|frames| frames as f64 / f64::from(params_rate.unwrap_or(rate)))
        .or_else(|| {
            let (duration, tb) = (track_duration?, time_base?);
            Some(
    ...
    shared.events.send(PlayerEvent::MetadataUpdated(Metadata {
        ...
        video_tracks: vec![],
        audio_tracks: vec![format!("audio/{channels}ch/{rate}Hz")],
        is_live,
        title,
    }));
    ...
    let target_samples = (PCM_TARGET_SECONDS * f64::from(rate)) as usize * CHANNELS as usize;
    {
        let mut pcm = lock(&shared.pcm);
    ...
    'main: loop {
        ...
        if let Some(target) = shared.take_pending_seek() {
            ...
            let time = Time::try_from_secs_f64(target).unwrap_or(Time::ZERO);
            match reader.seek(
                SeekMode::Accurate,
                SeekTo::Time {
                    time,
                ...
                Ok(_) => discard_until = Some(target),
        ...
        if want_play && device.is_none() && !open_failed {
            ...
            match Device::open(rate as f32, player_callback, userdata) {
        ...
        if at_eof {
            let mut pcm = lock(&shared.pcm);
            emit_position(shared, &mut pcm, rate, want_play);
            if pcm.queue.is_empty() && !eos_announced {
                eos_announced = true;
                // Land currentTime on the clip's end, not the last 250 ms tick,
                // before the element hears `ended`.
...

        {
            let mut pcm = lock(&shared.pcm);
            emit_position(shared, &mut pcm, rate, want_play);
            if pcm.queue.len() >= target_samples {
        ...
        if audio.spec().rate() != rate {
            log::info!("audio: sample rate changed mid-play; ending");
            at_eof = true;
            continue 'main;
        }

        let pkt_start = packet_secs(&packet.pts, time_base, rate);
        ...
        if let Some(target) = discard_until {
            let pkt_end = pkt_start + frames as f64 / f64::from(rate);
            if pkt_end <= target {
                continue 'main;
            }
            let skip = (((target - pkt_start) * f64::from(rate)).max(0.0) as usize).min(frames);
            samples = &samples[skip * channels..];
            discard_until = None;
        }

        let mut pcm = lock(&shared.pcm);
        push_stereo(&mut pcm, samples, channels);
        pcm.decoded_secs = pkt_start + frames as f64 / f64::from(rate);
    }
...
fn packet_secs(pts: &Timestamp, time_base: Option<TimeBase>, rate: u32) -> f64 {
    match time_base {
        Some(tb) => tb.calc_time_saturating(*pts).as_secs_f64(),
        // Audio timebases are essentially always 1/rate.
        None => pts.get() as f64 / f64::from(rate),
    }
}
...
mod tests {
    ...
    fn player(stream_type: StreamType) -> (SdlAudioPlayer, Arc<Mutex<Vec<PlayerEvent>>>) {
        ...
        let player = SdlAudioPlayer::with_sink(0, stream_type, Box::new(Collector(events.clone())));
        (player, events)
    }
    ...
    #[test]
    fn can_play_type_maps_symphonia_coverage() {
        let cases = [
            ...
            ("AUDIO/MPEG", SupportsMediaType::Probably),
            ("video/mp4", SupportsMediaType::No),
            ("audio/webm", SupportsMediaType::No),
            ("", SupportsMediaType::No),
        ];
        for (mime, expected) in cases {
            assert_eq!(can_play_type(mime), expected, "for {mime:?}");
        }
    }
