...
pub struct BrowserConfig {
    ...
    /// Reopen the tabs that were open at exit instead of starting on
    /// `home_page` (see [`crate::data::session`]). Off also drops the stored
    /// session, so it can't resurface later.
